...
Перейдем по ссылке до бакета, чтобы убедиться в доступности данных: https://storage.yandexcloud.net/arrow-datasets.
...
Воспользуемся функцией `String::SplitToList` (также можно использовать функцию `SUBSTRING`), чтобы разбить адрес в виде строки на список строк с разделителем `.`, применим функцию к `dst` и к `src` и выберем количество уникальных адресов.
...
Обратная задача, нам необходимо поменять `dst` и `src` местами в запросе:
